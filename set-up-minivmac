...
1. Either download the minivmac Zip file from this repo, or compile it yourself (covered in another file). There are two options available for download here; minivmac-trixie32-v37.zip launches in windowed mode, while minivmac-trixie32-v37-fs launches into fullscreen mode. They're identical in functionality otherwise, and are configured for 640x480 resolution. (If you want to customize this, you'll need to follow the guide to compile it on your own.)
...
6. To set up Mini vMac to launch automatically when the Pi boots (we're going to use SSH for this because it's easy and if you don't know much about the command line, this is a good opportunity to learn):
  Connect to the Pi using SSH. macOS, Windows and Linux all have this built into their respective command prompts (yes, even Windows!)
